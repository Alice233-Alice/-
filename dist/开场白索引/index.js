...
function toPreviewText(content) {
  return normalizeSpace(content).replace(/\*\*/g, "").replace(/```[\s\S]*?```/g, "").slice(0, 180);
}

function getGreetingItems() {
...
function findGreetingSwipeMessage() {
...
function extractBracketTitle(text) {
  const firstLine = text.split("\n").map(line => line.trim()).find(Boolean) ?? "";
  return firstLine.match(/^\[(.+?)\]/)?.[1]?.trim() ?? "";
}

function getFirstMessageContent() {
  const data = getCharData("current");
  if (!data) {
    return "";
  }
...
function resolveSwipeId(target, swipes) {
  ...
  const hasFirstMesAtZero = normalizedFirstMes.length > 0 && normalizedFirstSwipe.length > 0 && (normalizedFirstSwipe === normalizedFirstMes || normalizedFirstSwipe.includes(normalizedFirstMes.slice(0, 120)) || normalizedFirstMes.includes(normalizedFirstSwipe.slice(0, 120)));
...
async function jumpToGreeting(targetIndex) {
  ...
  await setChatMessages([ {
    message_id: greetingMessage.message_id,
    swipe_id: swipeId
  } ], {
    refresh: "all"
  });
...
function buildPopupContent(items, onPicked) {
  const $root = $("<div>").css({
    display: "grid",
    gap: "12px",
    maxHeight: "68vh",
  ...
  const $hero = $("<div>").css({
    ...
    borderRadius: "12px",
    padding: "10px 12px",
    background: "linear-gradient(135deg, rgba(24,42,72,0.75) 0%, rgba(20,57,78,0.55) 45%, rgba(18,32,58,0.75) 100%)",
  ...
  $root.append($hero);
  items.forEach(item => {
    const $card = $("<button>").attr("type", "button").css({
      textAlign: "left",
      width: "100%",
      border: "1px solid rgba(90,180,255,0.28)",
      borderRadius: "12px",
      padding: "10px 12px",
      background: "linear-gradient(145deg, rgba(17,35,60,0.9) 0%, rgba(22,53,82,0.75) 45%, rgba(13,30,52,0.88) 100%)",
...
async function openGreetingIndexPopup() {
  const items = getGreetingItems();
  if (items.length === 0) {
...
function ensureButtonVisible() {
  ...
  const next = hasButton ? current.map(button => button.name === SCRIPT_BUTTON_NAME ? {
    ...button,
    visible: true
  } : button) : [ ...current, {
    name: SCRIPT_BUTTON_NAME,
    visible: true
  } ];
...
function setupButtonRegistrationRetry() {
  let attempts = 0;
