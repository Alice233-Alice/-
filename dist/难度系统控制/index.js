...
(() => {
  ...
  \*****************************/
  __webpack_require__.r(__webpack_exports__);
  var _schema__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../踏月寻仙-测试版/schema */ "./src/踏月寻仙-测试版/schema.ts");
  const SCRIPT_NAME = "难度系统控制";
  let isWriting = false;
  let ignoreNextForMessageKey = null;
  const processedSignatures = new Set;
  function addProcessedSignature(signature) {
  ...
  function buildWarning(finalFactor) {
    if (finalFactor <= .82) return "天道垂青，风浪稍息，可稳步行事。";
    if (finalFactor <= .95) return "阻力稍缓，仍需谨慎，切莫贪进。";
    if (finalFactor < 1.08) return "天道运转如常，万物循理。";
    if (finalFactor < 1.2) return "天道隐有压迫，行事宜稳，勿露锋芒。";
    return "天道排斥加剧，强敌环伺，宜避其锋。";
  }
  function processDifficultyState(difficulty) {
    ...
    const baseline = snapshot.回合基线系数 || 1;
    const prevFinal = snapshot.本回合最终系数 || 1;
    let target = baseline;
    if (probe === "顺遂") target += .08;
    if (probe === "受挫") target -= .08;
    if (balance.生效剩余回合 > 0) target -= .12;
    target = clamp(target, .7, 1.3);
    const maxDelta = clamp(strategy.单回合改变量上限, .01, .3);
    let finalFactor = clamp(target, prevFinal - maxDelta, prevFinal + maxDelta);
    if (probe === "平稳") {
      const fallback = clamp(strategy.自然回落速度, .005, .2);
    ...
    snapshot.分层来源.世界叙事层 = 1;
    snapshot.分层来源.玩家偏好层 = 1;
    snapshot.分层来源.短期状态层 = clamp(finalFactor, .7, 1.3);
    snapshot.本回合最终系数 = clamp(finalFactor, .7, 1.3);
  ...
  async function handleDifficultyUpdate() {
    ...
    let parsed;
    try {
      parsed = _schema__WEBPACK_IMPORTED_MODULE_0__.Schema.parse(statData);
    } catch (e) {
