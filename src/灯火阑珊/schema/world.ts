...
export const FactionSchema = z
  .object({
    ...
    力: z
      ...
      .transform(value => value.trim())
      .prefault('未知'),
    营: z.enum(['正', '魔', '中']).catch('中').prefault('中'),
    模: z.enum(['超大', '大', '小', '微', '特']).catch('小').prefault('小'),
  })
  .prefault({});
...
export const TreasureSchema = z
  .object({
    阶: z.enum(['凡器', '法器', '灵器', '法宝', '灵宝', '仙器', '圣器', '道器', '本命']).catch('凡器').prefault('凡器'),
    类: z
      .string()
      .transform(value => value.trim())
      .prefault('未知'),
...
export const LocationSchema = z
  .object({
    ...
    特: z
      ...
      .transform(value => value.trim())
      .prefault('未记录'),
    资: NormalizedStringListSchema,
  })
  .prefault({});

// 灵根 Schema - 极简版（速度根据质自动计算）
export const SpiritRootSchema = z
  .object({
    质: z.enum(['劣', '下', '中', '上', '极', '天', '异']).catch('中').prefault('中'),
    性: z
      .string()
      .transform(value => value.trim())
      .prefault('未知'),
    稀: z.enum(['常', '少', '罕', '稀', '传']).catch('常').prefault('常'),
  })
  .prefault({})
  .transform(data => {
...
export const PhysiqueSchema = z
  .object({
    质: z.enum(['凡', '灵', '道', '圣', '神']).catch('凡').prefault('凡'),
    特: z
      .string()
      .transform(value => value.trim())
      .prefault('未记录'),
  ...
  .transform(data => ({
    ...data,
    优:
      data.质 === '神' ? '至高' : data.质 === '圣' ? '极强' : data.质 === '道' ? '强' : data.质 === '灵' ? '中' : '无',
  }));
