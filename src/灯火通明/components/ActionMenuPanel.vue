    ...
    <div class="actions-section">
      <div class="section-title">
        ...
        <div class="title-right">
          <span class="controller-dot" :class="{ connected: pseudo.controllerReady }" :title="pseudo.controllerReady ? '伪同层控制脚本已连接' : '伪同层控制脚本未连接'">
      ...
      <template v-else>
        ...
        <div class="actions-group support-group">
          <div class="group-title">
            <span>稳妥动作</span>
            <span class="group-meta">{{ supportActions.length + 3 }} 项</span>
          </div>
          ...
          <div class="permanent-actions">
            ...
            <div class="action-card custom-action" @click="selectCustomAction">
              <div class="action-icon icon-自定">
                <i class="fa-solid fa-pen-nib"></i>
              </div>
              <div class="action-info">
                <div class="action-header">
                  <span class="action-name">自定一念</span>
                  <span class="action-type-tag">自由</span>
                </div>
                <div class="action-desc">不受既有选项限制，亲自决定这一回合的言行与取舍。</div>
                <div class="action-meta">
                  <div class="reward">
...
const openComposer = (title: string, prompt: string) => {
...
const selectAction = (action: OpportunityAction) => {
  ...
  openComposer(action.名称, actionText);
...
.stop-command {
  ...
  border-color: color-mix(in srgb, var(--semantic-danger) 35%, transparent) !important;
...
.status-banner {
  ...
  &.info {
    ...
    border-color: color-mix(in srgb, var(--semantic-info) 18%, transparent);
...
.action-card {
  ...
  &.urgent {
    ...
    background: linear-gradient(135deg, var(--bg-primary) 0%, color-mix(in srgb, var(--semantic-danger) 10%, transparent) 100%);
  ...
  // NSFW 子类卡片边框发光
  &.nsfw-双修 {
    border-color: color-mix(in srgb, var(--semantic-relation) 40%, transparent);
    &:hover {
      border-color: var(--semantic-relation);
      box-shadow: 0 4px 16px color-mix(in srgb, var(--semantic-relation) 25%, transparent);
    }
  }
  &.nsfw-亲密 {
    border-color: color-mix(in srgb, var(--semantic-relation) 30%, transparent);
    &:hover {
      border-color: color-mix(in srgb, var(--semantic-relation) 78%, var(--jade));
      box-shadow: 0 4px 16px color-mix(in srgb, var(--semantic-relation) 20%, transparent);
    }
  }
  &.nsfw-调情 {
    ...
    &:hover {
      ...
      box-shadow: 0 4px 16px color-mix(in srgb, var(--crimson) 20%, transparent);
    }
  }
  ...
  &.story-action {
    ...
    background: linear-gradient(135deg, var(--bg-primary) 0%, color-mix(in srgb, var(--jade) 5%, transparent) 100%);

    &:hover {
      border-color: var(--jade);
      box-shadow: 0 4px 12px color-mix(in srgb, var(--jade) 15%, transparent);
    }
  }
...
.action-icon {
  ...
  // 不同类型的颜色
  &.icon-探索 {
    background: linear-gradient(135deg, var(--semantic-success), color-mix(in srgb, var(--semantic-success) 70%, var(--surface-inset)));
  }
  &.icon-任务 {
    background: linear-gradient(135deg, var(--semantic-info), color-mix(in srgb, var(--semantic-info) 70%, var(--surface-inset)));
  }
  &.icon-交易 {
    background: linear-gradient(135deg, var(--gold), var(--semantic-warning));
  }
  &.icon-结交 {
    background: linear-gradient(135deg, var(--jade-soft), var(--jade));
  }
  &.icon-争夺 {
    background: linear-gradient(135deg, var(--semantic-danger), var(--crimson));
  }
  &.icon-修炼 {
    background: linear-gradient(135deg, var(--jade), var(--semantic-info));
  }
  &.icon-红颜 {
    background: linear-gradient(135deg, var(--semantic-relation), var(--crimson));
  }
  // NSFW 子类图标样式
  &.icon-nsfw-双修 {
  ...
  &.icon-随机 {
    background: linear-gradient(135deg, var(--text-secondary), color-mix(in srgb, var(--text-secondary) 65%, var(--surface-inset)));
  }
  &.icon-剧情 {
    background: linear-gradient(135deg, var(--jade-soft), color-mix(in srgb, var(--jade) 65%, var(--surface-inset)));
  }
  &.icon-自定 {
    background: linear-gradient(135deg, var(--accent-color), #7c8a6d);
  }
}
