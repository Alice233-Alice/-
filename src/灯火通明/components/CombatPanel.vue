<template>
  <!-- 渡劫状态 -->
  <TribulationPanel v-if="isInTribulation" :tribulation-state="tribulationState" />
...
const isInCombat = computed(() => {
  ...
  if (['对峙', '试探', '交锋', '决胜', '脱战'].includes(props.combatState?.阶段 ?? '')) return true;
